/**
 * @file port_cpu.c
 * @brief ARM Cortex-M3 port CPU implementation for SertOS.
 *
 * Implements hardware stack frame initialization with Thumb-2 architecture,
...
typedef struct CortexM3StackFrame {
    uint32_t r4;
    uint32_t r5;
    uint32_t r6;
    uint32_t r7;
...
void* sertos_port_stack_init(void* stack_top, void* stack_limit, SertosTaskFunction entry, void* param)
{
    ...
    top_addr -= sizeof(CortexM3StackFrame);

    frame = (CortexM3StackFrame*)top_addr;
